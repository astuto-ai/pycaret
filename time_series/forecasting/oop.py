...
from onelens.onelens_pycaret.containers.metrics.time_series import get_all_metric_containers
from onelens.onelens_pycaret.containers.models.time_series import (
    ALL_ALLOWED_ENGINES,
    get_all_model_containers,
    get_container_default_engines,
)
from onelens.onelens_pycaret.internal.display import CommonDisplay
from onelens.onelens_pycaret.internal.distributions import get_base_distributions
...
from onelens.onelens_pycaret.internal.plots.time_series import _get_plot
from onelens.onelens_pycaret.internal.plots.utils.time_series import (
    _clean_model_results_labels,
    _get_data_types_to_plot,
    _reformat_dataframes_for_plots,
    _resolve_renderer,
)
from onelens.onelens_pycaret.internal.preprocess.time_series.forecasting.preprocessor import (
    TSForecastingPreprocessor,
)
from onelens.onelens_pycaret.internal.pycaret_experiment.ts_supervised_experiment import (
    _TSSupervisedExperiment,
)
from onelens.onelens_pycaret.internal.tests.time_series import (
...
from onelens.onelens_pycaret.utils.datetime import coerce_datetime_to_period_index
from onelens.onelens_pycaret.utils.generic import MLUsecase, _resolve_dict_keys, highlight_setup
from onelens.onelens_pycaret.utils.time_series import (
    TSApproachTypes,
    TSExogenousPresent,
    TSModelTypes,
    auto_detect_sp,
...
from onelens.onelens_pycaret.utils.time_series.forecasting import (
    PyCaretForecastingHorizonTypes,
    _check_and_clean_coverage,
    get_predictions_with_intervals,
    update_additional_scorer_kwargs,
)
from onelens.onelens_pycaret.utils.time_series.forecasting.model_selection import (
    ForecastingGridSearchCV,
    ForecastingRandomizedSearchCV,
    cross_validate,
)
from onelens.onelens_pycaret.utils.time_series.forecasting.models import DummyForecaster
from onelens.onelens_pycaret.utils.time_series.forecasting.pipeline import (
...
class TSForecastingExperiment(_TSSupervisedExperiment, TSForecastingPreprocessor):
    ...
    def _check_and_clean_index(
            ...
            if index in self.data.columns.to_list():
                ...
                if unique_index_before and not unique_index_after:
                    raise ValueError(
                        ...
                        " Consider setting the data index outside onelens. onelens_pycaret before passing to setup()."
                    )
        ...
        if len(self.data.index) != len(set(self.data.index)):
            raise ValueError(
                "Index may not have duplicate values! Please check and correct before passing to onelens. onelens_pycaret"
            )
        ...
        if isinstance(self.data.index, pd.PeriodIndex):
            ...
            if len(self.data.index) != len(expected_idx):
                ...
                raise ValueError(
                    ...
                    "\nPlease add missing indices to data before passing to onelens. onelens_pycaret."
                    ...
                    "\n>>> from onelens.onelens_pycaret.utils.time_series import clean_time_index"
    ...
    def setup(
        ...
        >>> from onelens.onelens_pycaret.time_series import *
    ...
    def tune_model(
        ...
        **kwargs:
        ...
        search_library = "onelens_pycaret"  # only 1 library supported right now

        _allowed_search_algorithms = []
        if search_library == "onelens_pycaret":
        ...
        # {'mae': <onelens_pycaret.containers....783DEB0C8>, 'rmse': <onelens_pycaret.containers....783DEB148> ...}
        ...
        elif search_library == "onelens_pycaret":
    ...
    def blend_models(
        ...
        """
        msg = (
            "method 'voting' is not supported from onelens.onelens_pycaret 3.0.1 onwards. "
    ...
    def _predict_model_reconcile_X(
        ...
        # returned by onelens. onelens_pycaret). Hence converting to None.
    ...
    def load_model(
        ...
        Example
        -------
        >>> from onelens.onelens_pycaret.time_series import load_model
    ...
    def _get_cleaned_estimator_y_X(
        ...
        """Some models like Prophet train on DatetimeIndex, but onelens. onelens_pycaret stores
    ...
    def _is_estimator_finalized(self, estimator: BaseForecaster) -> bool:
        ...
        if len(estimator_y) == len(self.y_train) and np.all(
            ...
            raise ValueError(
                ...
                "\nhttps://github.com/onelens_pycaret/onelens_pycaret/issues/new/choose"
            )
        ...
        if insample_predictions is not None:
            ...
            if self.verbose:
                print(
                    ...
                    "this is implemented, it will be enabled by default in onelens. onelens_pycaret."
                )
