...
import onelens.onelens_pycaret.loggers
from onelens.onelens_pycaret.internal.display import CommonDisplay
from onelens.onelens_pycaret.internal.logging import create_logger, get_logger, redirect_output
from onelens.onelens_pycaret.internal.memory import get_memory
from onelens.onelens_pycaret.internal.pipeline import Pipeline as InternalPipeline
from onelens.onelens_pycaret.internal.plots.helper import MatplotlibDefaultDPI
from onelens.onelens_pycaret.internal.plots.yellowbrick import show_yellowbrick_plot
from onelens.onelens_pycaret.internal.pycaret_experiment.pycaret_experiment import _PyCaretExperiment
from onelens.onelens_pycaret.internal.validation import is_sklearn_cv_generator
from onelens.onelens_pycaret.loggers.base_logger import BaseLogger
from onelens.onelens_pycaret.loggers.comet_logger import CometLogger
from onelens.onelens_pycaret.loggers.dagshub_logger import DagshubLogger
from onelens.onelens_pycaret.loggers.mlflow_logger import MlflowLogger
from onelens.onelens_pycaret.loggers.wandb_logger import WandbLogger
...
class _TabularExperiment(_PyCaretExperiment):
    ...
    def _get_groups(
        ...
        return onelens. onelens_pycaret.utils.generic.get_groups(groups, data, fold_groups)
    ...
    def _get_cv_splitter(
        ...
        return onelens. onelens_pycaret.utils.generic.get_cv_splitter(
            fold,
    ...
    def _get_model_id(self, e, models=None) -> str:
        ...
        return onelens. onelens_pycaret.utils.generic.get_model_id(e, models)
    ...
    def _convert_log_experiment(
    ...
    ) -> Union[bool, onelens.onelens_pycaret.loggers.DashboardLogger]:
        if not (
            (
        ...
        if log_experiment:
            ...
            if loggers_list:
                return onelens. onelens_pycaret.loggers.DashboardLogger(loggers_list)
        return False
    ...
    def _initialize_setup(
        ...
        """
        This function initializes the environment in onelens. onelens_pycaret. setup()
        must be called before executing any other function in onelens. onelens_pycaret.
        It takes only two mandatory parameters: data and name of the
        target column.

        """
        from onelens.onelens_pycaret import __version__
    ...
    def _plot_model(
        ...
        estimator,
        plot: str = "auc",
        scale: float = 1,  # added in onelens. onelens_pycaret==2.1.0
        ...
        with patch(
            ...
            onelens. onelens_pycaret.internal.patches.yellowbrick.is_estimator,
        ):
            with patch(
                ...
                def residuals_interactive():
                    from onelens.onelens_pycaret.internal.plots.residual_plots import (
    ...
    def deploy_model(
        ...
        model_name: str,
        authentication: dict,
        platform: str = "aws",  # added gcp and azure support in onelens. onelens_pycaret==2.1
    ):
        """
        (In Preview)
        ...
        >>> experiment_name = setup(data = juice,  target = 'Purchase')
        >>> lr = create_model('lr')
        >>> deploy_model(model = lr, model_name = 'deploy_lr', platform = 'aws', authentication = {'bucket' : 'onelens_pycaret-test'})
        ...
        return onelens. onelens_pycaret.internal.persistence.deploy_model(
    ...
    def save_model(
        ...
        **kwargs,
        ...
        model_, model_filename = onelens. onelens_pycaret.internal.persistence.save_model(
            model=model,
    ...
    def load_model(
        ...
        authentication : dict
            ...
            When platform = 'gcp':
            {'project': 'gcp_pycaret', 'bucket' : 'onelens_pycaret-test'}

            When platform = 'azure':
            {'container': 'onelens_pycaret-test'}
        ...
        model = onelens. onelens_pycaret.internal.persistence.load_model(
...
import pandas as pd
from onelens.onelens_pycaret.{self._ml_usecase.name.lower()} import load_model, predict_model
